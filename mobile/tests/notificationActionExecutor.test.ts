...
test.each([42, "42"])("sends the bound button id through the authenticated dispatcher for message %s", async (message_id) => {
  const request = jest.fn().mockResolvedValueOnce(response({ outcome: "recorded" }));
  expect(await executeNotificationAction({ ...data, message_id }, "agora.action.1", async () => credentials, request)).toBe("recorded");
...
test.each([42, "42"])("reconciles a lost POST response for message %s", async (message_id) => {
  ...
  expect(await executeNotificationAction({ ...data, message_id },"agora.action.0",async () => credentials,request)).toBe("recorded");
...
test("a concurrent identical submission is success, a different choice is already handled", async () => {
  for (const choice of ["allow", "deny"]) {
    ...
    expect(await executeNotificationAction({ ...data, message_id: "42" },"agora.action.0",async () => credentials,request))
